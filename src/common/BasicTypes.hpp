...
using ClOrdId       = std::uint64_t; // identifies client order id from trading system to broker/exch
using OrderId       = std::uint64_t; // identifies an order within a strategy on the market
using StrategyId    = std::uint16_t; // identifies a strategy
using MarketId      = std::uint16_t; // identifies a market/exchange
using SecurityId    = std::uint16_t; // identifies fungible security traded on 1 or more exchanges
