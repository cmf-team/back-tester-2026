#include "AbseilOrderBook.hpp"

namespace cmf
{

void AbseilOrderBook::add_to_level(Side side, ScaledPrice price, ScaledPrice delta)
{
    auto& level = side == Side::Buy ? bids_[price] : asks_[price];
    level += delta;
}

void AbseilOrderBook::remove_from_level(Side side, ScaledPrice price,
                                        ScaledPrice delta)
{
    if (side == Side::Buy)
    {
...
void AbseilOrderBook::apply_impl(const MarketDataEvent& e)
{
    switch (e.action)
    {
    case Action::Add:
...
std::optional<ScaledPrice>
AbseilOrderBook::best_price_impl(Side side) const noexcept
{
    if (side == Side::Buy)
...
uint64_t AbseilOrderBook::volume_at_impl(Side side,
...
bool AbseilOrderBook::empty_impl(Side side) const noexcept
{
    switch (side)
    {
    case Side::Buy:
...
std::span<const AbseilOrderBook::LevelPair>
AbseilOrderBook::side_levels_impl(Side side) const
{
    levels_cache_.clear();
    if (side == Side::Buy)
    {
