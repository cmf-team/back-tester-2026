...
static bool argv_contains_as_csv(int argc, char** argv) noexcept {
    for (int i = 1; i < argc; ++i) {
        if (std::string_view(argv[i]) == "--as-csv") {
            return true;
        }
    }
    return false;
...
static int run_as_csv_replay(int argc, char** argv) {
    const char* csv_path = "MD/trades.csv";
    bool use_micro = false;

    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--as-csv") == 0) {
            continue;
        }
        if (std::strcmp(argv[i], "--micro") == 0) {
            ...
            std::printf(
                "Usage: %s --as-csv [path/to/trades.csv] [--micro]\n"
...
static int run_mbo_json(int argc, char** argv) {
...
int main(int argc, char** argv) {
    if (argv_contains_as_csv(argc, argv)) {
        return run_as_csv_replay(argc, argv);
    }
    return run_mbo_json(argc, argv);
}