...
bool map_action(std::string_view action, EventType& out) {
    if (action == "A") { out = EventType::Add;    return true; }
    if (action == "C") { out = EventType::Cancel; return true; }
    if (action == "M") { out = EventType::Modify; return true; }
    if (action == "T") { out = EventType::Trade;  return true; }
    if (action == "F") { out = EventType::Fill;   return true; }
    if (action == "R") { out = EventType::Reset;  return true; }
    return false;
}
...
uint64_t parse_iso_ts(std::string_view s) {
    ...
    auto g2 = [&](size_t p) -> uint64_t { return (s[p]-'0')*10+(s[p+1]-'0'); };
    ...
    uint64_t nanos=0; int nd=0;
    if (s.size()>20 && s[19]=='.') {
        for (size_t i=20; i<s.size()&&s[i]>='0'&&s[i]<='9'&&nd<9;++i,++nd)
            nanos=nanos*10+(s[i]-'0');
        while(nd<9){nanos*=10;++nd;}
    }
    static const uint16_t dim[]={0,31,28,31,30,31,30,31,31,30,31,30,31};
    auto leap=[](uint64_t y){return(y%4==0&&y%100!=0)||(y%400==0);};
    uint64_t days=0;
    for(uint64_t y=1970;y<year;++y) days+=leap(y)?366:365;
    for(uint64_t m=1;m<month;++m){days+=dim[m];if(m==2&&leap(year))++days;}
    days+=day-1;
...
int64_t parse_price_str(std::string_view s) {
    ...
    if(s[i]=='-'){neg=true;++i;}
    int64_t intg=0;
    while(i<s.size()&&s[i]>='0'&&s[i]<='9') intg=intg*10+(s[i++]-'0');
    int64_t frac=0; int fd=0;
    if(i<s.size()&&s[i]=='.'){
        ++i;
        while(i<s.size()&&s[i]>='0'&&s[i]<='9'&&fd<9){frac=frac*10+(s[i++]-'0');++fd;}
        while(fd<9){frac*=10;++fd;}
    }
...
bool parse_line(std::string_view line, MarketDataEvent& out) {
    ...
    if (doc["action"].get(action)) return false;
    EventType type{};
    if (!map_action(action, type)) return false;

    // side
    Side side = Side::Bid;
    if (type != EventType::Reset) {
        std::string_view side_sv;
        if (doc["side"].get(side_sv)) return false;
        if      (side_sv == "B") side = Side::Bid;
        else if (side_sv == "A") side = Side::Ask;
        else return false;
    }

    // ts_recv — top-level ISO8601 string
    uint64_t ts_recv = 0;
    { std::string_view v; if (!doc["ts_recv"].get(v)) ts_recv = parse_iso_ts(v); }
    ...
    uint64_t ts_event = 0, instrument_id = 0;
    {
        ...
        if (!doc["hd"].get_object().get(hd)) {
            std::string_view v;
            if (!hd["ts_event"].get(v)) ts_event = parse_iso_ts(v);
            uint64_t iid = 0;
            if (!hd["instrument_id"].get(iid)) instrument_id = iid;
        }
    }

    // price — string "0.021200000" or null
    int64_t price = 0;
    { std::string_view v; if (!doc["price"].get(v)) price = parse_price_str(v); }

    // size
    int64_t qty = 0;
    { int64_t v=0; if (!doc["size"].get(v)) qty = v; }

    // order_id — string containing uint64
    uint64_t order_id = 0;
    { std::string_view v; if (!doc["order_id"].get(v)) order_id = parse_u64_str(v); }

    // flags
    uint8_t flags = 0;
    { uint64_t v=0; if (!doc["flags"].get(v)) flags = static_cast<uint8_t>(v); }
