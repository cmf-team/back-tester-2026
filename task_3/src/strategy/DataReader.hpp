...
inline std::vector<Trade> read_trades(const std::string& path) {
    std::ifstream f(path);
    if (!f.is_open()) throw std::runtime_error("Cannot open trades: " + path);
    ...
    while (std::getline(f, line)) {
        ...
        if (ts.empty() || price.empty()) continue;
        Trade t;
...
inline std::vector<BookSnapshot> read_lob(const std::string& path, int depth = 25) {
    std::ifstream f(path);
    if (!f.is_open()) throw std::runtime_error("Cannot open lob: " + path);
