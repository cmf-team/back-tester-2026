...
public:
    ...
    void on_book(const BookSnapshot& snap,
        ...
        if (bid_price >= snap.best_ask()) bid_price = snap.best_ask() - snap.spread() * 0.1;
        if (ask_price <= snap.best_bid()) ask_price = snap.best_bid() + snap.spread() * 0.1;
...
private:
    void update_vol(double mid) {
        ...
        if ((int)mid_history_.size() < 2) return;
